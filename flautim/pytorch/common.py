...
class Measures(object):
    ...
    def log(self, experiment, metric, values, validation = False, epoch = None, **append):
        ts = str(datetime.now())
        data = {"Experiment": self.context.IDexperiment, 
                "user": experiment.model.suffix, 
                "timestamp": ts,
                "metric" : str(metric), 
                "model" : experiment.model.uid, 
...
def run_centralized(experiment, name_log = 'centralized.log', post_processing_fn = [], **kwargs):
    ...
    ctx.log("Starting Centralized Training", details="", object="experiment_run", object_id=experiment_id )
    ...
    try:
        ...
        copy_model_weights(path, output_path, experiment_id, logger) 
...
class Context:
    def __init__(self):
        ...
        self.backend = Backend(        ######## self.backend ou backend? Mas aí não é global, chamamos fora como?
            server=self.args.dbserver,
            port=self.args.dbport,
            user=self.args.dbuser,
            password=self.args.dbpw
        )

        self.logger = Logger(self.backend, self.args)
        self.measures = Measures(self.backend, self.args)

    def _add_arguments(self):
        self.parser.add_argument("--user", type=str, required=True)
        self.parser.add_argument("--path", type=str, required=True)
        self.parser.add_argument("--output-path", type=str, required=True)
        self.parser.add_argument("--dbserver", type=str, default="127.0.0.1")
        self.parser.add_argument("--dbport", type=str, default="27017")
        self.parser.add_argument("--dbuser", type=str, required=True)
        self.parser.add_argument("--dbpw", type=str, required=True)
        self.parser.add_argument("--clients", type=int, default=3)
        self.parser.add_argument("--rounds", type=int, default=10)
        self.parser.add_argument("--epochs", type=int, default=10)
        self.parser.add_argument("--IDexperiment", type=str, required=True)
...
def copy_model_weights(path, output_path, id, logger):
    try:
        ...
        logger.log("Erro while copying model weights", details=str(e), object="filesystem_file", object_id=id )
